...
RSpec.describe "Valid search", type: :system do
  ...
  let(:record_date_of_birth) { "2000-01-15" }
  let(:search_month) { ["January", "Jan", "jan", "january"].sample }  # Stringified month to test the edge case
  ...
  def and_there_is_a_record
    @record = create(:childrens_barred_list_entry, date_of_birth: record_date_of_birth)
  end
  ...
  def and_i_enter_their_date_of_birth
    ...
    fill_in("Month", with: search_month)
